...
| 2026-06-03 | Claude Code (Opus 4.8) | 瞬時スペクトラムを専用イベント`SpectrumFrame`に分離（上限10kHz＝Nyquistでクランプ、スペクトログラムは6400のまま）。固定対数軸 10Hz〜10kHz に。低音域が広がりすぎる問題を解消 |
| 2026-06-03 | Claude Code (Opus 4.8) | スペクトラム軸の微調整: 左余白を除去（データ最下点に軸下限を合わせる）＋目盛りを素のHz表記に（カスタム`_HzLogAxis`で指数表記を回避） |
| 2026-06-03 | Claude Code (Opus 4.8) | READMEを現行機能に合わせ全面整理（冒頭で6可視化を提示） |
| 2026-06-03 | Claude Code (Opus 4.8) | **声区・声傾向推定を追加**(`analysis/voice_profile`): F0+F1+HNR(Parselmouth)+スペクトル重心で 地声/ミックス/裏声・低声/中間/高声 を推定（信頼度付き、librosa/pyworld不要）。1秒カデンスで`VoiceProfileSample`。**UI刷新**: 数値情報を左の縦パネルにリスト表示、グラフは右に縦積み。verify_pipelineのpauseチェックを残留整定後カウントに修正（イベント増対応）。全検証合格 |
| 2026-06-03 | Claude Code (Opus 4.8) | 声傾向ラベルを男声寄り/中声/女声寄りに変更（性ではなく声の傾向）。推定の確度を「（確度：高/中/低）」表記に |

### v1.6 開発中（段階実装・都度確認）
方針: i18n=ライブ切替 / レポート=pyqtgraph描画 / 録音=独立ボタン。順序 A→E。
...
### v1.6後の改善
- 2026-06-04: 声の傾向の比重をユーザー指定で再配分: **ピッチ:フォルマント ≈ 3.5:6.5**（F0 ±1.6 / 母音正規化F1F2 ±2.0(gain10) / F3 ±1.0 / 重心 +0.4）。フォルマント群単独で確度「高」到達可、逆向きフォルマントはゾーン外F0も打ち消せる。全検証合格。
- 2026-06-04: **声の傾向推定を刷新**（ユーザー指摘: F1項の符号が音声学と逆＋実質ピッチのみで判定していた）。新スコア: F0(±2.2) と**フォルマント群(±2.3)を同等重み**に。①母音正規化F1/F2リーン＝推定母音の男声/女声参照値へのlog距離差（±1.5, gain 8）、②F3声道長キュー（±0.8, 2300〜3400Hzバンド探索でスロットずれに頑健）、③重心(+0.4)。判定|score|≥1.4、確度高≥3.0（**F0単独では確度高に届かない**設計）。検証: 同一F0=175Hz（重複帯）で男声/女声フォルマントのみで判定が分かれることを合成音で確認。合成音声のノウハウ（5フォルマント・HPノイズ・基本波補強・整数周期F0）はverifyのdocstringに記録。
