...
- 🎵 **ピッチ（F0）** — スクロール式グラフ＋数値。通常/拡張(C7)レンジ切替
...
- 🧭 **声区・声の傾向の推定** — 地声寄り／ミックス／裏声寄り、低声寄り／高声寄りを推定（目安）
...
- **推定** — **声区**（地声寄り／ミックス／裏声寄り）と**声の傾向**（低声寄り／中間／高声寄り）を信頼度付きで推定、HNR も表示。
  - 推定は F0・F1・HNR・スペクトル重心などから算出する**目安**です（断定ではありません）。マイクや個人差で変動し、持続発声で安定します。
  - “性別”ではなく**声の音響的な傾向**として表示しています（声だけから性別は判定できないため）。
