...
- **スペクトログラム（狭帯域／広帯域）** — スクロール式ウォーターフォール。狭帯域は倍音が横縞に、広帯域はフォルマント帯が見えます。
- **オシロスコープ（時間波形）** — 入力波形をリアルタイム表示。
- **瞬時スペクトラム（FFT）** — 対数周波数軸で今この瞬間の周波数成分を折れ線表示。ピークホールド対応。
- **表示パネルのカスタマイズ** — View メニューで6種（ピッチ／フォルマント／オシロ／瞬時スペクトラム／狭帯域SG／広帯域SG）の表示をそれぞれ切替。
- **設定の保存** — レンジ・音量・表示パネル・デバイス選択を保存し、次回起動時に復元。
- フォルマント／スペクトログラムの表示は **6.4 kHz** まで対応。
